...
const ConnectionPerformanceTest: React.FC = () => {
    ...
    try {
      ...
      for (let i = 0; i < 5; i++) {
        const note = {
          ...
          height: 120,
          zIndex: 1000 + i,
          color: ["yellow", "green", "blue", "purple", "pink"][i] as
            | "yellow"
            | "green"
            | "blue"
            | "purple"
  ...
  return (
    <div style={{ padding: "20px", maxWidth: "800px" }}>
      ...
      <Text type="secondary">用于测试和验证连接线性能监控功能的开发工具</Text>
      ...
      <Card title="测试操作" style={{ marginTop: "16px" }}>
        <Space wrap>
          ...
          <Button onClick={createTestConnections} loading={isRunningTest}>
            🔗 创建测试连接
          </Button>
          <Button onClick={runPerformanceStressTest} loading={isRunningTest}>
            🚀 性能压力测试
          </Button>
          <Button danger onClick={cleanupTestData}>
            🧹 清理测试数据
          </Button>
        </Space>
      </Card>
